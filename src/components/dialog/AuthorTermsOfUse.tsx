...
export default function AuthorTermsOfUse({ isOpen, onClose }: Props) {
  ...
  const [contractImgSrc, setContractImgSrc] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    const loadLatestContractImage = async () => {
      try {
        setContractLoading(true);
        setContractError(null);
        setContractImgSrc(null);

        const storage = getStorage(); // dùng default firebase app đã init
        ...
        // Lấy metadata để chọn ảnh mới nhất (updated/timeCreated)
        ...
        withMeta.sort((a, b) => b.time - a.time);

        const latestRef: StorageReference = withMeta[0].ref;
        const url = await getDownloadURL(latestRef);

        if (!cancelled) setContractImgSrc(url);
      } catch (e: any) {
        if (!cancelled) {
      ...
      >
        ...
        >
          ...
          <div className="max-h-[75vh] overflow-auto pr-2">
            ...
            {!contractLoading && !contractError && contractImgSrc && (
              <img
                src={contractImgSrc}
                alt="Hợp đồng tác giả"
                className="w-full h-auto rounded-md border border-white/10 object-contain"
                loading="lazy"
              />
            )}
          </div>

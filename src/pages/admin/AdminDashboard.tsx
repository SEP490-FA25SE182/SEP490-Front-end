  ...
  for (const o of orders) {
    ...
    switch (filter) {
      case "day": {
        ...
        const index = Math.floor(hour / 4); // 0–5
        label = labels[index];
        break;
      }
      ...
      case "quarter": {
        const diffMonth = now.getMonth() - d.getMonth();
        if (diffMonth >= 0 && diffMonth <= 2) {
          label = labels[2 - diffMonth];
        }
        break;
      }
...
export default function AdminDashboardPage() {
  ...
  const [bookStatsMap, setBookStatsMap] = useState<Map<string, BookStats>>(
    new Map()
  );
  ...
  const [monthlyRevenueMap, setMonthlyRevenueMap] = useState<
    Record<number, number>
  >({});
  const [revenueByOrderId, setRevenueByOrderId] = useState<
  ...
  const allStats = useMemo(
    () => Array.from(bookStatsMap.values()),
    [bookStatsMap]
  );
              ...
              <CardContent className="h-80">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={monthlyRevenueProfit}>
                    ...
                    <Legend
                      wrapperStyle={{ color: "#161515ff", fontWeight: 600 }}
                    />
